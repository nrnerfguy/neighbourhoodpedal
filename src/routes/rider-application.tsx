...
function RiderApplicationPage() {
  ...
  if (loading || checking) {
    return <div className="min-h-dvh grid place-items-center bg-[var(--silver)] text-sm text-muted-foreground">Loading…</div>;
  }
  if (!user) {
    return (
      <div className="min-h-dvh bg-[var(--silver)] grid place-items-center p-6">
        <div className="text-center space-y-3">
          <p>You need to sign in to apply.</p>
          <Link to="/auth" className="rounded-2xl bg-primary text-[var(--forest)] font-bold px-5 py-2.5 inline-block">Sign in</Link>
        </div>
      </div>
    );
  }
